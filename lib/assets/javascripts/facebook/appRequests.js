M.provide("facebook", new function () {
   ...
   * @param access_token
   */
  that.deleteAppRequests = function (compoundRequestIds, access_token) {
  ...
  that.sendAppRequests = function (options) {
    ...
    var fbUiHash = {
      ...
      to:facebookUserIds.join(",")
    };

    if (options['display']) fbUiHash.display = options['display'];

    FB.ui(fbUiHash, function (request) {
